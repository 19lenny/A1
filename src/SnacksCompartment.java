public class SnacksCompartment {
    ...
    public void refill(int amount, String filling){
        ...
        //If the refiller wants to put more Snack in the machine as remaining capacity, the method should stop
        ...
        //the Snack is created from the name the refiller gets us
        //the vending machine keeps track on how many items are refilled and fills up the stock in the first empty spot.
        //the vending machine can only fill up as many snacks the user wants to refill
        else{
            int filled = 0;
            for (int i=0; i<stock.length; i++){
                if (filled >= amount){
    ...
    public Snack[] dispender(int amount, String name){
        ...
        //starting with a helping counter, to define the position in which the vending machine has to give out the snack.
        int counter = 0;
